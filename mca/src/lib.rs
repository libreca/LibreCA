...
use common::{Number, u_vec, UVec, ValueGenerator};
use sut::Solver;
...
impl<ValueId: Number, LocationsType: Number> MCA<ValueId, LocationsType> {
    ...
    ///
    /// let mca: MCA<usize, u8> = MCA::<usize, u8>::new_unconstrained::<usize, 2>(&parameters);
    ///
    ...
    >(
    ...
    ) -> MCA<ValueId, LocationsType> where [(); STRENGTH - 1]:, [(); STRENGTH - 2]: {
        ...
        let mut dont_care_locations = u_vec![LocationsType::mask_high(STRENGTH); array.len()];
        dont_care_locations.reserve(capacity - array.len());
        dont_care_locations[0] = LocationsType::default();
    ...
    pub fn set_vertical_extension_rows(&mut self, at_parameter: usize) -> LocationsType {
        ...
        let locations_mask = LocationsType::mask_low(at_parameter);
        debug_assert!((locations_mask & LocationsType::bit(at_parameter)).none());
        debug_assert!((locations_mask & LocationsType::bit(at_parameter - 1)).any());

        self.vertical_extension_rows.clear();
        for (index, locations) in self.dont_care_locations.iter().enumerate() {
            if (locations_mask & *locations).any() {
    ...
    pub fn append_row<ParameterId: Number, const STRENGTH: usize>(
        &mut self,
        at_parameter: usize,
    ...
    pub fn check_ve_rows(&self, at_parameter: usize) -> bool {
        ...
        if self.dont_care_locations.is_empty() {
            ...
            let dont_care_mask = LocationsType::mask_low(at_parameter);

            let mut ve_rows = self.vertical_extension_rows.iter();
            for (index, locations) in self.dont_care_locations.iter().enumerate() {
                if (dont_care_mask & *locations).any() {
                    let item = ve_rows
                        .next()
...
pub fn pc_to_mask<ParameterId: Number, LocationsType: Number, const STRENGTH: usize>(
    pc: &[ParameterId; STRENGTH - 1],
    at_parameter: usize,
) -> (LocationsType, LocationsType) {
    let mut result = LocationsType::default();
    for parameter in pc.iter() {
        result += LocationsType::bit(parameter.as_usize());
    }
    (result, result + LocationsType::bit(at_parameter))
}

/// During debugging this method can check the don't care locations of the row.
pub fn check_locations<ValueId: Number, LocationsType: Number>(row: &[ValueId], locations: LocationsType) -> bool {
    assert!(
